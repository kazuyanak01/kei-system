...
def check_mismatch(old_s, old_d, cur_s, cur_d):
    ...
    if cur_s == '芝':
        if (o in [1,2] and n in [3,4,5]) or (o == 3 and n in [4,5]) or (o == 5 and n != 5): return True
    else: # ダート
        if (o == 1 and n != 1) or (o in [2,3] and n in [4,5,6,7]) or (o in [6,7] and n not in [6,7]): return True
    return False
    ...
    if not input_text:
        ...
        try:
            ...
            for i in range(1, len(blocks), 2):
                ...
                # 馬名抽出
                name_part = re.split(r'(東京|中山|中京|京都|阪神|新潟|小倉|福島|札幌|函館|芝|ダ)', content)[0]
                name_l = [l.strip() for l in name_part.split('\n') if l.strip() and not any(m in l for m in ['◎','◯','▲','△','☆','消','✓','&#10003'])]
                ...
                # もしcandsが空ならmax_1yを採用するが、その場合もペナルティ対象かチェック
                if not cands:
                    ref = max_1y # フォールバック
                else:
                    ref = max(cands)
                ...
                processed_data.append({'num': num, 'name': h_name, 'ref': ref, 'linear': linear, 'kei': linear})
